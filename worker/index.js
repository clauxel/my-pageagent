const LIVE_ORIGIN = 'https://pageagent.space'
const LIVE_HOST = 'pageagent.space'
const ALT_HOSTS = new Set(['www.pageagent.space'])
...
const planCatalog = {
  starter: {
    ...
    currency: 'USD',
    summary: 'one-workflow PageAgent Space pilot',
  },
  pro: {
    id: 'pro',
    name: 'Pro',
    ...
    currency: 'USD',
    summary: 'default production Page Agent launch plan',
  },
  scale: {
    id: 'scale',
    name: 'Scale',
    ...
    currency: 'USD',
    summary: 'extension, MCP, and governance rollout',
  },
}
...
function resolveConfiguredProductId(env, planId, billing) {
  ...
  const keys = [
    ...
    `CREEM_PRODUCT_ID_PAGEAGENT_${normalizedSelection}`,
...
async function getOrCreateCreemProduct(env, apiKey, plan, billing, successUrl) {
  ...
  const product = await requestCreemJson(apiKey, `${resolveCreemBase(env)}/v1/products`, {
    name: `PageAgent Space ${plan.name} (${billingLabel})`,
...
async function handleCheckout(request, env, requestUrl) {
  ...
  try {
    ...
    const checkout = await requestCreemJson(apiKey, `${resolveCreemBase(env)}/v1/checkouts`, {
      ...
      request_id: `pageagent_${Date.now()}_${Math.random().toString(16).slice(2)}`,
      metadata: {
        site: 'pageagent.space',
        planId: plan.id,
        billing,
      },
    })
...
async function handleAnalytics(request) {
  ...
  console.log(JSON.stringify({ type: 'analytics', site: 'pageagent.space', accepted: events.length, events }))
